...
const ProjectCard = (props) => {
  ...
  return (
    data?.length > 0 && data.map(item => {
      ...
      return (
        <div key={item.id} className='bg-white p-8 shadow-xl rounded-lg mb-6 flex flex-col md:flex-row justify-between lg:items-center'>
          <div className='flex flex-1 items-center'>
            <div className='flex items-center justify-center h-20 w-20 rounded-full mr-4'>
              <img src={item.project_image} alt="" className='w-full h-full rounded-full' />
            </div>
              ...
              <div className='flex items-center gap-1 mt-2'>
                <div className='text-gray-600 text-sm w-[200px] border border-gray-300 h-[9px] relative'>
                  <div className={`absolute top-0 left-0 bottom-0 bg-text-main_green h-[9px]`}
                    style={{
                      // width: item?.number_of_people ? item?.number_of_people / item.number_programmers * 100 + '%' : '0%',
                ...
                {/* <span className='font-gunterz-bold text-[12px]'>({item.number_of_people ? item?.number_of_people + ' / ' + item.number_programmers : 0 + ' / ' + item.number_programmers})</span> */}

                {/* <span className='font-gunterz-bold text-[12px]'>({item.number_of_people ? item?.number_of_people / item.number_programmers * 100 + '%' + ' / ' + item.number_programmers : 0 + '%' + ' / ' + item.number_programmers})</span> */}
              </div>

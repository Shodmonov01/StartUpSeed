                    ...
                    <div key={item.id} className='flex items-center justify-between relative'>
                        <div className='flex flex-1 items-center'>
                            <div onClick={() => navigateHandler(item)}
                                ...
                                <div
                                    className='flex items-center justify-center h-[80px] w-[80px] text-white mr-4 rounded-full overflow-hidden'
                                // style={{ backgroundImage: `url(${item.project_image})`, backgroundSize: "cover", backgroundPosition: "center", backgroundRepeat: "no-repeat" }}
                                >
                                    <img
                                        src={item.project_image}
                                        alt='no image'
                                        // className='w-[80px] h-[80px] rounded-full bg-transparent border-none'
                        ...
                        <div className='absolute top-[30%] right-4'>
                            {item.favorite ? (
                                <IoStarSharp className='text-3xl text-main-green cursor-pointer' onClick={() => deleteFavorite(item)} />
                            ) : (
                                <IoStarOutline className='text-3xl text-gray-500 cursor-pointer' onClick={() => addFavorite(item)} />
                            )}
                        </div>
                    </div>
                )
